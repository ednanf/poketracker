...
describe('Save Files Routes', () => {
    ...
    describe('GET /', () => {
        it('returns `200` and calls `getAllSaveFiles` controller', async () => {
            const response = await request(app)
                .get('/api/v1/save-file');

            expect(response.status)
                .toBe(StatusCodes.OK);
            expect(saveFilesController.getAllSaveFiles)
                .toHaveBeenCalled();
        });
    });
    ...
    describe('POST /', () => {
        it('returns `201` and calls `createSaveFile` controller with valid data', async () => {
            ...
            expect(saveFilesController.createSaveFile)
                .toHaveBeenCalled();
        });
    ...
    describe('GET /:id', () => {
        it('returns `200` with a valid 24-character ID', async () => {
            ...
            expect(response.status)
                .toBe(StatusCodes.OK);
            expect(saveFilesController.getSaveFileById)
                .toHaveBeenCalled();
        });

        it('returns `400` when ID format is invalid (length !== 24)', async () => {
            const response = await request(app)
                .get('/api/v1/save-file/123');
    ...
    describe('PATCH /:id', () => {
        it('returns `200` and updates with valid data', async () => {
            ...
            expect(response.status)
                .toBe(StatusCodes.OK);
            expect(saveFilesController.updateSaveFile)
                .toHaveBeenCalled();
        });
    ...
    describe('DELETE /:id', () => {
        it('returns `200` and calls deleteSaveFile controller', async () => {
            ...
            expect(response.status)
                .toBe(StatusCodes.OK);
            expect(saveFilesController.deleteSaveFile)
                .toHaveBeenCalled();
        });
    });
    ...
    describe('PATCH /:id/sync', () => {
        it('returns `200` and calls syncSaveFile with valid payload', async () => {
            ...
            expect(response.status)
                .toBe(StatusCodes.OK);
            expect(saveFilesController.syncSaveFile)
                .toHaveBeenCalled();
        });
